(function ($) {
    ...
    var halo = {
        ...
        ready: function () {
            ...
            // // 当按钮被点击时触发
            // $('#md-btn__form__onSubmit').click(function(event){
            //     event.preventDefault();
            //     event.stopPropagation();
            //     window.location.href = 'https://goblacklyte.com';
            // });
        },
            ...
            if ($(window).width() > 1025) {
                ...
                $(document).off('click.toggleSearch', iconSearchMenu).on('click.toggleSearch', iconSearchMenu, function (event) {
                    ...
                    if (window.location.pathname === '/' && $(window).scrollTop() === 0) {
                        $('.shopify-section.shopify-section-group-header-group.section-header-navigation').addClass('scrolled-past-header')
                    }
                });
                ...
                $(document).off('click.hideSearchSticky').on('click.hideSearchSticky', function (event) {
                    ...
                    if ($('[class*="section-header-"]').hasClass('sticky-search-menu-open') && !formSearch.has(event.target).length && !quickSearch.has(event.target).length) {
                        ...
                        $('body').css('overflow', 'auto')
                        if (window.location.pathname === '/' && $(window).scrollTop() === 0) {
                            $('.shopify-section.shopify-section-group-header-group.section-header-navigation').removeClass('scrolled-past-header')
                        }
        ...
        menuSidebarMobile: function () {
            ...
            $doc.on('click', '[data-mobile-menu-level2-chairs]', function (e) {
                $('.chairs_collections_mb').addClass('menu_open_mb_active')
                $body.addClass('menu_open_mb')
            })

            // 点击移动端的desk menu
            $doc.on('click', '[data-mobile-menu-level2-desk]', function (e) {
                $('.header-nav-desks-sections').addClass('menu_open_desk_mb_active')
                $body.addClass('menu_open_mb')
            })
        },
        ...
        initMobileMenuDefault: function (url) {
            ...
            if (navHtml) {
                ...
                const secondLi = tempDiv.find('li:nth-child(2)');
                const allLi = tempDiv.find('li')
                allLi.each(function (index) {
                    const li = $(this)
                    const originalLink = li.find('a').first(); // 获取原有 <a>，只取第一个
                    let href = originalLink.length ? originalLink.attr('href') : 'javascript:void(0);'; // 保持原有 href

                    // 单独处理第二个li
                    if (index === 1) {
                        // 将 data-mobile-menu-level2-chairs 添加到 <li> 标签上
                        ...
                        li.append(`
                    <a href="javascript:void(0);" data-mobile-menu-level2-chairs class="menu-lv-1__action header__menu-item header__menu-item list-menu__item link link--text focus-inset link-effect">
                      <span class="text p-relative"> > </span>
                    </a>
                  `)
                    } else if (index === 2) {
                        // 单独处理第三个li, desk. 将data-mobile-menu-level2-desk 添加到li标签上
                        ...
                        li.append(`
                    <a href="javascript:void(0);" data-mobile-menu-level2-desk class="menu-lv-1__action header__menu-item header__menu-item list-menu__item link link--text focus-inset link-effect">
                      <span class="text p-relative"> > </span>
                    </a>
                  `)
                    } else {
                        // **所有 <li> 统一插入一个新的 <a>，href 保持原有的**
                        li.append(`
                      <a href="${href}" class="menu-lv-1__action header__menu-item list-menu__item link link--text focus-inset link-effect">
                          <span class="text p-relative"> > </span>
                      </a>
                  `);
                    }
            ...
            $('.video-open-popup .video-button').off('click').on('click', function () {
                ...
                const $content = `<div class="fluid-width-video-wrapper" style="padding-top: ${aspect_ratio}">
                        ...
                        `<iframe
                                            ...
                                            mozallowfullscreen
                                            allowFullScreen
                                            src="https://www.youtube.com/embed/${video_src}?autoplay=1&mute=1;controls=0">
                                        </iframe>`
                        :
                        `<iframe 
                                            src="https://player.vimeo.com/video/${video_src}?autoplay=1&mute=1;controls=0" 
                                            class="js-vimeo" 
                                            allow="autoplay; 
                                            encrypted-media" 
                                            webkitallowfullscreen 
        ...
        productImageGallery: function ($scope) {
            ...
            var detailsFancybox = $scope.find('[data-fancybox-details]')

            if (detailsFancybox.length > 0) {
                                ...
                                const player = sliderNav.find('.slick-slide.slick-active').data('youtube-player'),
                                    ...
                                    videoContent = `<div class="fluid-width-video-wrapper" style="padding-top: 56.24999999999999%">
                                            ...
                                            `<iframe
                                                            ...
                                                            mozallowfullscreen
                                                            allowFullScreen
                                                            src="https://www.youtube.com/embed/${videoUrl}?autoplay=1&mute=1&start=${dataTime};controls=0">
                                                        </iframe>`
                                            :
                                            `<iframe 
                                                            src="https://player.vimeo.com/video/${videoUrl}?autoplay=1&mute=1;controls=0" 
                                                            class="js-vimeo" 
                                                            allow="autoplay; 
                                                            encrypted-media" 
                                                            webkitallowfullscreen 
        ...
        initCountdown: function () {
            ...
            if (countdownElm.length) {
                countdownElm.each(function () {
                    ...
                    window.countdownfunctionInit = countdownfunction
                });
            }
        ...
        initCollapseSidebarBlock: function () {
            $doc.on('click', '.sidebarBlock-headingWrapper .sidebarBlock-heading', (event) => {
                ...
                // 判断是否有已经打开的页签
                const isAnyItemOpen = $('.sidebarBlock-headingWrapper .sidebarBlock-heading.is-clicked').length > 0;
                if (isAnyItemOpen) {
                    $('.sidebarBlock-headingWrapper .sidebarBlock-heading').not($target).removeClass('is-clicked');
                    $('.sidebarBlock-contentWrapper').not($blockCollapse).stop(true, true).slideUp('slow').css('display', 'none');
                }
